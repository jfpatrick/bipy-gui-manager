...
from .conftest import new_project_parameters
...
def test_collect_most_common_right_values(mock_phonebook, tmpdir):
    parameters = new_project_parameters(name="test-project", desc="A test project", author="me", path=tmpdir,
...
def test_collect_credentials_cernid_discovered(monkeypatch, mock_phonebook, tmpdir):
    ...
    parameters = new_project_parameters(name="test-project", desc="A test project", path=tmpdir,
                                           repo_type="test", gitlab_token="skip-authentication")
    new_params = project_info.collect(parameters)
    assert new_params["author_cern_id"] == "me"
    assert new_params["author_full_name"] == "Test User"
...
def test_collect_credentials_cernid_given_wrong(monkeypatch, mock_phonebook, tmpdir):
    parameters = new_project_parameters(name="test-project", desc="A test project", author="you", interactive=False,
...
def test_collect_credentials_cernid_discovered_wrong(monkeypatch, mock_phonebook, tmpdir):
    ...
    parameters = new_project_parameters(name="test-project", desc="A test project", interactive=False, path=tmpdir,
                                           repo_type="test", gitlab_token="skip-authentication")
    with pytest.raises(ValueError):
        project_info.collect(parameters)
...
def test_collect_name_not_given_ask(monkeypatch, mock_phonebook, tmpdir):
    ...
    monkeypatch.setattr('builtins.input', lambda _: 1/0)
    parameters = new_project_parameters(desc="A test project", author="me", repo_type="test", path=tmpdir,
...
def test_collect_name_no_underscores(mock_phonebook, tmpdir):
    ...
    parameters = new_project_parameters(name="test_project", desc="A test project", author="me", path=tmpdir,
                                           repo_type="test", interactive=False, gitlab_token="skip-authentication")
    with pytest.raises(ValueError):
        project_info.collect(parameters)
...
def test_collect_name_no_not_alphanumeric(mock_phonebook, tmpdir):
    ...
    parameters = new_project_parameters(name="te$t(project)", desc="A test project", author="me", path=tmpdir,
                                           repo_type="test", interactive=False, gitlab_token="skip-authentication")
    with pytest.raises(ValueError):
        project_info.collect(parameters)
...
def test_collect_name_no_uppercase(mock_phonebook, tmpdir):
    ...
    parameters = new_project_parameters(name="TestProject", desc="A test project", author="me", path=tmpdir,
                                           repo_type="test", interactive=False, gitlab_token="skip-authentication")
    with pytest.raises(ValueError):
        project_info.collect(parameters)
...
def test_collect_name_no_empty_string(monkeypatch, mock_phonebook, tmpdir):
    ...
    parameters = new_project_parameters(name="", desc="A test project", author="me", path=tmpdir,
                                           repo_type="test", interactive=False, gitlab_token="skip-authentication")
    with pytest.raises(ValueError):
        project_info.collect(parameters)
...
def test_collect_desc_valid(mock_phonebook, tmpdir):
    # Right values
    parameters = new_project_parameters(name="test-project", desc="A test project", author="me", repo_type="test",
...
def test_collect_desc_not_given_ask(monkeypatch, mock_phonebook, tmpdir):
    # If not given as parameter will ask
    monkeypatch.setattr('builtins.input', lambda _: 1/0)
    parameters = new_project_parameters(name="test-project", author="me", repo_type="test", path=tmpdir,
                                           gitlab_token="skip-authentication")
    with pytest.raises(ZeroDivisionError):
        project_info.collect(parameters)
...
def test_collect_desc_no_empty_string(monkeypatch, mock_phonebook, tmpdir):
    # Empty string not allowed
    parameters = new_project_parameters(name="test-project", desc="", author="me", repo_type="test", path=tmpdir,
...
def test_collect_project_path_specified_exists(monkeypatch, tmpdir, mock_phonebook):
    ...
    monkeypatch.setattr('builtins.input', lambda _: 1 / 0)
    parameters = new_project_parameters(name="test-project", desc="A test project", author="me",
...
def test_collect_project_path_empty_string(monkeypatch, mock_cwd, mock_phonebook, tmpdir):
    ...
    monkeypatch.setattr('builtins.input', lambda _: 1 / 0)
    os.chdir(tmpdir)
    parameters = new_project_parameters(name="test-project", desc="A test project", author="me", path="",
                                           repo_type="test", gitlab_token="skip-authentication")
    new_params = project_info.collect(parameters)
...
def test_collect_project_path_not_specified(monkeypatch, mock_cwd, mock_phonebook):
    ...
    monkeypatch.setattr('builtins.input', lambda _: 1 / 0)
    parameters = new_project_parameters(name="test-project", desc="A test project", author="me", path=None,
                                           repo_type="test", gitlab_token="skip-authentication")
    with pytest.raises(ZeroDivisionError):
        project_info.collect(parameters)
...
def test_collect_repo_type_operational(monkeypatch, mock_phonebook, tmpdir):
    parameters = new_project_parameters(name="test-project", desc="A test project", author="me", interactive=False,
