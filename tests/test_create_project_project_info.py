...
def test_collect_most_common_right_values(mock_phonebook):
    ...
    assert new_params["author_full_name"] == "Me"
...
def test_collect_name_not_given_ask(monkeypatch):
...
def test_collect_name_not_given_ask_get_valid(monkeypatch):
...
def test_collect_name_no_underscores():
...
def test_collect_name_no_whitespace():
...
def test_collect_name_no_not_alphanumeric():
...
def test_collect_name_no_uppercase():
...
def test_collect_name_no_empty_string(monkeypatch):
...
def test_collect_desc_not_given_ask_get_valid(monkeypatch):
    # If given through CLI, should save it
...
def test_collect_desc_no_double_quotes():
...
def test_collect_desc_no_empty_string(monkeypatch):
    # Empty string not allowed - will fail
...
def test_collect_desc_not_given(monkeypatch):
    # If not given as parameter will ask
    monkeypatch.setattr('builtins.input', lambda _: 1/0)
...
# #         Path          #
# #########################
def test_collect_project_path_use_base_path_validation(monkeypatch, mock_cwd):
...
def test_collect_project_path_empty_string(monkeypatch, mock_cwd):
    monkeypatch.setattr('builtins.input', lambda _: 1 / 0)
    parameters = create_project_parameters(name="test-project", desc="A test project", author="Me", path="",
                                           email="me@cern.ch", repo="https://:@gitlab.cern.ch:8443/user/project.git")
...
def test_collect_project_path_not_specified(monkeypatch, mock_cwd):
    monkeypatch.setattr('builtins.input', lambda _: 1 / 0)
...
def test_collect_project_path_specified_and_existing(monkeypatch, tmpdir):
...
def test_collect_project_path_specified_not_existing(monkeypatch, tmpdir):
...
# #    Not Interactive    #
# #########################
def test_collect_not_interactive_all_values_given(monkeypatch):
...
def test_collect_not_interactive_not_all_values_given(monkeypatch):
    monkeypatch.setattr('builtins.input', lambda _: 1 / 0)
...
def test_collect_not_interactive_values_given_wrong(monkeypatch):
    monkeypatch.setattr('builtins.input', lambda _: 1 / 0)
    parameters = create_project_parameters(name="test-project", desc="A test project", author="Me",
